...
import { getHighlightsFilePath } from "@/next-lib/userData/paths";
...
export async function GET(request: Request) {
  ...
  const items = readItemList(getHighlightsFilePath(userId, hash));
...
export async function POST(request: Request) {
  ...
  const hash = resolveBookIdentity(manifestUrl);
  upsertItem(getHighlightsFilePath(userId, hash), item);
...
export async function DELETE(request: Request) {
  ...
  const hash = resolveBookIdentity(manifestUrl);
  removeItem(getHighlightsFilePath(userId, hash), id);
