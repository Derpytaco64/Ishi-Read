...
import { CURRENT_USER_ID, ensureUsersRegistry, getHighlightsFilePath } from "@/next-lib/userData/paths";
...
export async function GET(request: Request) {
  ...
  const items = readItemList(getHighlightsFilePath(CURRENT_USER_ID, hash));
...
export async function POST(request: Request) {
  ...
  ensureUsersRegistry();
  const hash = resolveBookIdentity(manifestUrl);
  upsertItem(getHighlightsFilePath(CURRENT_USER_ID, hash), item);
...
export async function DELETE(request: Request) {
  ...
  const hash = resolveBookIdentity(manifestUrl);
  removeItem(getHighlightsFilePath(CURRENT_USER_ID, hash), id);
