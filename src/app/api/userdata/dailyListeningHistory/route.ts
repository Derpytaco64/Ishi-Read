...
import { getDailyListeningHistoryFilePath } from "@/next-lib/userData/paths";
...
export async function GET(request: Request) {
  ...
  const buckets = readJsonFile<DailyListeningBucket[]>(getDailyListeningHistoryFilePath(userId, hash));
...
export async function POST(request: Request) {
  ...
  const hash = resolveBookIdentity(manifestUrl);
  writeJsonFileAtomic(getDailyListeningHistoryFilePath(userId, hash), buckets);
