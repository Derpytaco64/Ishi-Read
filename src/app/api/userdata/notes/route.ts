...
import { CURRENT_USER_ID, ensureUsersRegistry, getNotesFilePath } from "@/next-lib/userData/paths";
...
export async function GET(request: Request) {
  ...
  const items = readItemList(getNotesFilePath(CURRENT_USER_ID, hash));
...
export async function POST(request: Request) {
  ...
  ensureUsersRegistry();
  const hash = resolveBookIdentity(manifestUrl);
  upsertItem(getNotesFilePath(CURRENT_USER_ID, hash), item);
...
export async function DELETE(request: Request) {
  ...
  const hash = resolveBookIdentity(manifestUrl);
  removeItem(getNotesFilePath(CURRENT_USER_ID, hash), id);
