...
import { getNotesFilePath } from "@/next-lib/userData/paths";
...
export async function GET(request: Request) {
  ...
  const items = readItemList(getNotesFilePath(userId, hash));
...
export async function POST(request: Request) {
  ...
  const hash = resolveBookIdentity(manifestUrl);
  upsertItem(getNotesFilePath(userId, hash), item);
...
export async function DELETE(request: Request) {
  ...
  const hash = resolveBookIdentity(manifestUrl);
  removeItem(getNotesFilePath(userId, hash), id);
